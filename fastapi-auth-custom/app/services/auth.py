...
from app.schemas.auth import TokenResponse,EmailSchema
...
async def verified_email(token,db):
    ...
    user_db=db.query(User).filter(User.email == user.get("email")).first()
    if not user_db:
...
async def forgot_password_user(email:EmailSchema):
    try:
        token_data ={
            "email":email
        }
